import React, { useState } from 'react';
import { SidebarProps } from '../types';
import { useTheme } from '../context/ThemeContext';
  

const Sidebar: React.FC<SidebarProps> = ({ onSelect, activeScreen, toggleSidebar }) => {
  const { theme, toggleTheme } = useTheme();
  return (
    <div className={`w-full h-screen absolute top-0 z-50 lg:relative ${theme === 'dark' ? 'bg-[#1A202C] text-white' : 'bg-white text-black'}`}>
        <div className='lg:hidden h-12 flex items-center justify-end border border-b px-5'>
            <svg onClick={toggleSidebar} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6 text-white font-extrabold">
                <path fillRule="evenodd" d="M5.47 5.47a.75.75 0 0 1 1.06 0L12 10.94l5.47-5.47a.75.75 0 1 1 1.06 1.06L13.06 12l5.47 5.47a.75.75 0 1 1-1.06 1.06L12 13.06l-5.47 5.47a.75.75 0 0 1-1.06-1.06L10.94 12 5.47 6.53a.75.75 0 0 1 0-1.06Z" clipRule="evenodd" />
            </svg>
        </div>
        <div className="p-5 hidden lg:block">
            <h1>Logo</h1>
        </div>
        <div className="mt-3 py-5 flex flex-col lg:items-center gap-3">
            <div
              onClick={() => onSelect('weather')}
              className={`w-full flex lg:items-center lg:justify-center gap-2 text-slate-400 pl-3 lg:pl-0 py-4 cursor-pointer ${activeScreen === 'weather' ? 'bg-slate-900 border-l-4' : 'hover:bg-slate-900 hover:border-l-4'}`}
            >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-4">
                    <path fillRule="evenodd" d="M4.5 9.75a6 6 0 0 1 11.573-2.226 3.75 3.75 0 0 1 4.133 4.303A4.5 4.5 0 0 1 18 20.25H6.75a5.25 5.25 0 0 1-2.23-10.004 6.072 6.072 0 0 1-.02-.496Z" clipRule="evenodd" />
                </svg>
                <h1 className="text-xs font-semibold">Weather</h1>
            </div>
            {/* <div
              onClick={() => onSelect('cities')}
              className={`w-full flex lg:items-center lg:justify-center gap-2 text-slate-400 pl-3 lg:pl-0 py-4 cursor-pointer ${activeScreen === 'cities' ? 'bg-slate-900 border-l-4' : 'hover:bg-slate-900 hover:border-l-4'}`}
            >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-4">
                    <path fillRule="evenodd" d="M3 2.25a.75.75 0 0 0 0 1.5v16.5h-.75a.75.75 0 0 0 0 1.5H15v-18a.75.75 0 0 0 0-1.5H3ZM6.75 19.5v-2.25a.75.75 0 0 1 .75-.75h3a.75.75 0 0 1 .75.75v2.25a.75.75 0 0 1-.75.75h-3a.75.75 0 0 1-.75-.75ZM6 6.75A.75.75 0 0 1 6.75 6h.75a.75.75 0 0 1 0 1.5h-.75A.75.75 0 0 1 6 6.75ZM6.75 9a.75.75 0 0 0 0 1.5h.75a.75.75 0 0 0 0-1.5h-.75ZM6 12.75a.75.75 0 0 1 .75-.75h.75a.75.75 0 0 1 0 1.5h-.75a.75.75 0 0 1-.75-.75ZM10.5 6a.75.75 0 0 0 0 1.5h.75a.75.75 0 0 0 0-1.5h-.75Zm-.75 3.75A.75.75 0 0 1 10.5 9h.75a.75.75 0 0 1 0 1.5h-.75a.75.75 0 0 1-.75-.75ZM10.5 12a.75.75 0 0 0 0 1.5h.75a.75.75 0 0 0 0-1.5h-.75ZM16.5 6.75v15h5.25a.75.75 0 0 0 0-1.5H21v-12a.75.75 0 0 0 0-1.5h-4.5Zm1.5 4.5a.75.75 0 0 1 .75-.75h.008a.75.75 0 0 1 .75.75v.008a.75.75 0 0 1-.75.75h-.008a.75.75 0 0 1-.75-.75v-.008Zm.75 2.25a.75.75 0 0 0-.75.75v.008c0 .414.336.75.75.75h.008a.75.75 0 0 0 .75-.75v-.008a.75.75 0 0 0-.75-.75h-.008ZM18 17.25a.75.75 0 0 1 .75-.75h.008a.75.75 0 0 1 .75.75v.008a.75.75 0 0 1-.75.75h-.008a.75.75 0 0 1-.75-.75v-.008Z" clipRule="evenodd" />
                </svg>
                <h1 className="text-xs font-semibold">Cities</h1>
            </div> */}
            <div
              onClick={() => onSelect('location')}
              className={`w-full flex lg:items-center lg:justify-center gap-2 text-slate-400 pl-3 lg:pl-0 py-4 cursor-pointer ${activeScreen === 'location' ? 'bg-slate-900 border-l-4' : 'hover:bg-slate-900 hover:border-l-4'}`}
            >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-4">
                    <path fillRule="evenodd" d="m11.54 22.351.07.04.028.016a.76.76 0 0 0 .723 0l.028-.015.071-.041a16.975 16.975 0 0 0 1.144-.742 19.58 19.58 0 0 0 2.683-2.282c1.944-1.99 3.963-4.98 3.963-8.827a8.25 8.25 0 0 0-16.5 0c0 3.846 2.02 6.837 3.963 8.827a19.58 19.58 0 0 0 2.682 2.282 16.975 16.975 0 0 0 1.145.742ZM12 13.5a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" clipRule="evenodd" />
                </svg>
                <h1 className="text-xs font-semibold">Location</h1>
            </div>
            <div
              onClick={() => onSelect('settings')}
              className={`w-full flex lg:items-center lg:justify-center gap-2 text-slate-400 pl-3 lg:pl-0 py-4 cursor-pointer ${activeScreen === 'settings' ? 'bg-slate-900 border-l-4' : 'hover:bg-slate-900 hover:border-l-4'}`}
            >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-4">
                    <path fillRule="evenodd" d="M11.078 2.25c-.917 0-1.699.663-1.85 1.567L9.05 4.889c-.02.12-.115.26-.297.348a7.493 7.493 0 0 0-.986.57c-.166.115-.334.126-.45.083L6.3 5.508a1.875 1.875 0 0 0-2.282.819l-.922 1.597a1.875 1.875 0 0 0 .432 2.385l.84.692c.095.078.17.229.154.43a7.598 7.598 0 0 0 0 1.139c.015.2-.059.352-.153.43l-.841.692a1.875 1.875 0 0 0-.432 2.385l.922 1.597a1.875 1.875 0 0 0 2.282.818l1.019-.382c.115-.043.283-.031.45.082.312.214.641.405.985.57.182.088.277.228.297.35l.178 1.071c.151.904.933 1.567 1.85 1.567h1.844c.916 0 1.699-.663 1.85-1.567l.178-1.072c.02-.12.114-.26.297-.349.344-.165.673-.356.985-.57.167-.114.335-.125.45-.082l1.02.382a1.875 1.875 0 0 0 2.28-.819l.923-1.597a1.875 1.875 0 0 0-.432-2.385l-.84-.692c-.095-.078-.17-.229-.154-.43a7.614 7.614 0 0 0 0-1.139c-.016-.2.059-.352.153-.43l.84-.692c.708-.582.891-1.59.433-2.385l-.922-1.597a1.875 1.875 0 0 0-2.282-.818l-1.02.382c-.114.043-.282.031-.449-.083a7.49 7.49 0 0 0-.985-.57c-.183-.087-.277-.227-.297-.348l-.179-1.072a1.875 1.875 0 0 0-1.85-1.567h-1.843ZM12 15.75a3.75 3.75 0 1 0 0-7.5 3.75 3.75 0 0 0 0 7.5Z" clipRule="evenodd" />
                </svg>
                <h1 className="text-xs font-semibold">Settings</h1>
            </div>
        </div>
        <div className="absolute bottom-20 left-1/2 transform -translate-x-1/2 w-[10%] border">
        <button onClick={toggleTheme} className="text-white text-xs font-semibold">
          Toggle {theme === 'dark' ? 'Light' : 'Dark'} Mode
        </button></div>
    </div>
  );
};

export default Sidebar;
